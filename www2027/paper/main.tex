...
\begin{table*}[t]
\caption{Broader development baseline comparison (validation-only MAP@100). Nonparametric rows are deterministic; learned rows report five-seed mean $\pm$ sample standard deviation. $^{*}$ marks an exact one-sided paired improvement over the corresponding frozen anchor at $p=0.03125$.}
\label{tab:dev}
\centering
\scriptsize
...
\begin{figure*}[t]
  ...
  \caption{Visual diagnostics replacing dense result tables. (a) Validation-only component gains over the anchors. (b) Protected anchor-hit opportunities retained on the sealed test; all violations are zero. (c) Sealed-test MAP@$K$ gains before (solid) and after (dashed) safety fusion; fusion costs 0.040--0.131 percentage points while keeping every gain positive.}
...
The evidence supports a narrow deployment claim: past-only adaptation can improve future-period diffusion ranking without target-period activity or deleting protected successes. Future work should test inductive candidates and exposure constraints on additional Web platforms.
